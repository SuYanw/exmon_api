defmodule ExMon.TrainerTest do
  ...
  describe "changeset/1" do
    test "when all params are valid, return a valid changeset" do
      ...
      assert %Ecto.Changeset{action: nil, changes: %{name: "Glaubert", password: "123456"}, errors: [], data: %ExMon.Trainer{}, valid?: true} = response
    end
    ...
    test "when there are invalid params, return a invalid changeset" do
      ...
      assert %Ecto.Changeset{action: nil, changes: %{password: "123456"},data: %ExMon.Trainer{}, valid?: false} = response
  ...
  describe "build/1" do
    test "when all params are valid, return a trainer struct" do
      ...
      assert {:ok, %Trainer{name: "Glaubert", password: "123456"}} = response
      end

      test "when all params are valid, return an error" do
        params = %{password: "123456"}
